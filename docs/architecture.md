...
Accepted product rule: Element Manager maps only `fuchsia-pkg://fuchsia.com/fuchsia_terminal#meta/fuchsia_terminal.cm` into the dedicated `terminal_elements` collection. That collection receives the bounded Starnix Controller route plus graphics, keyboard/IME, tracing, logging, and Inspect capabilities. The bridge starts Alpine Bash and installs only the embedded `fuchsia-studio` help/manual assets into writable `/usr/local`. Generic `elements` do not receive the Controller route or Terminal process authority; a live Flatland-Rainbow control element verifies the negative boundary.

Bidirectional acceptance uses a fixed one-shot Workbench child with only `fuchsia.ui.test.input.Registry`. It types `fuchsia-studio help\n`; Live 22 visibly shows retained health lines, the typed command, the complete unwrapped narrative, and the returned prompt. The input driver is not a general text-injection service and grants no new authority to Terminal. The accepted compressed screenshot SHA-256 is `21fea48ac054103800c5815a660c50a8aa7b225ce2cc151beba64843a2ced28f`.
